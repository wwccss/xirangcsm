<?php
/**
 * The api library of XiRangCSM.
 *
 * @copyright   Copyright 2009-2011 QingDao Nature Easy Soft Network Technology Co,LTD (www.cnezsoft.com)
 * @license     LGPL (http://www.gnu.org/licenses/lgpl.html)
 * @author      Chunsheng Wang <chunsheng@cnezsoft.com>
 * @package     API
 * @version     $Id: api.class.php 1406 2011-02-17 14:14:03Z wwccss $
...
class ztclient
{
    ...
    private function setControlAPI($module, $method, $vars = '')
    {
        ...
        {
            $controlAPI = $this->appendSession($this->zentao->root);
            $controlAPI .= "&{$this->zentao->moduleVar}=$module&{$this->zentao->methodVar}=$method&{$this->zentao->viewVar}=json$vars";
        }
        elseif($this->zentao->requestType == 'PATH_INFO')
        {
            ...
            $controlAPI = $this->appendSession($controlAPI);
        }
        return $controlAPI;
    }
    ...
    private function setModelAPI($module, $method, $vars = '')
    {
        ...
        if($this->zentao->requestType == 'GET')
        {
            $modelAPI = $this->appendSession($this->zentao->root);
            $modelAPI .= "&{$this->zentao->moduleVar}=api&{$this->zentao->methodVar}=getModel&{$this->zentao->viewVar}=json";
            $modelAPI .="&module=$module&method=$method&params=$vars";
        }
        elseif($this->zentao->requestType == 'PATH_INFO')
        {
            $modelAPI  = $this->zentao->root . 'api' . $this->zentao->requestFix . 'getmodel' . $this->zentao->requestFix;
            $modelAPI .= $module . $this->zentao->requestFix . $method . $this->zentao->requestFix;
            $modelAPI .= $vars;
            $modelAPI  = rtrim($modelAPI, $this->zentao->requestFix) . '.json';
            $modelAPI  = $this->appendSession($modelAPI);
        }
        return $modelAPI;
    }
    ...
    public function login()
    {
        ...
        $authHash = md5($this->zentao->password . $this->session->rand);
        $loginAPI .= "&account={$this->zentao->account}&password=$authHash";
    ...
    private function appendSession($url)
    {
        ...
        if($this->session) $url .= '&' . $this->session->sessionName . '=' . $this->session->sessionID;
